...
<script>
  // Supabase Authentication + checkAuthState defined early
...
<script>
  // Supabase Authentication Handling (login/signup/logout)
  const authModal = document.getElementById("auth-modal");
  const loginButton = document.getElementById("login-button");
  const authClose = document.getElementById("auth-close");
  const authToggleBtn = document.getElementById("auth-toggle-btn");
  ...
  document.addEventListener("click", function toggleSwitch(e) {
    if (e.target && e.target.id === "auth-toggle-btn") {
      if (loginForm.style.display !== "none") {
        loginForm.style.display = "none";
        signupForm.style.display = "flex";
        document.getElementById("auth-modal-title").innerText = "Create an Account";
        document.getElementById("auth-toggle-text").innerHTML = 'Already have an account? <span class="auth-link" id="auth-toggle-btn">Sign in</span>';
      } else {
        loginForm.style.display = "flex";
        signupForm.style.display = "none";
        document.getElementById("auth-modal-title").innerText = "Login to Your Account";
        document.getElementById("auth-toggle-text").innerHTML = 'Don\'t have an account? <span class="auth-link" id="auth-toggle-btn">Sign up</span>';
      }
    }
  });
  ...
  signupForm?.addEventListener("submit", async (e) => {
    ...
    if (error) {
      ...
      authMessage.innerText = "Signup successful! Please check your email to confirm.";
      authMessage.className = "auth-message success";
      authMessage.style.display = "block";
      setTimeout(() => {
        loginForm.style.display = "flex";

...
<script>
/* --- AUTH UI BINDINGS (attached after DOM is ready) --- */
document.addEventListener('DOMContentLoaded', function() {
  ...
  const githubBtn = $('github-auth');
  const googleBtn = $('google-auth');
  const discordBtn = $('discord-auth');
  const logoutBtn = $('logout-btn');

  // Open modal with specific mode: 'login' | 'signup'
  function openAuthModal(mode = 'login') {
    ...
    if (mode === 'signup') {
      ...
      authToggleText.innerHTML = 'Already have an account? <span class="auth-link" id="auth-toggle-btn">Sign in</span>';
    } else {
      loginForm.style.display = 'flex';
      signupForm.style.display = 'none';
      document.getElementById('auth-modal-title').innerText = 'Login to Your Account';
      authToggleText.innerHTML = "Don't have an account? <span class='auth-link' id='auth-toggle-btn'>Sign up</span>";
    }
    clearAuthMessage();
    authModal.classList.add('active');
  }
  ...
  // Toggle link inside modal (using event delegation)
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async function(e) {
      ...
      try {
        ...
        console.error(err);
        showAuthMessage('Unexpected error during login', 'error');
      } finally {
  ...
  if (signupForm) {
    signupForm.addEventListener('submit', async function(e) {
      ...
      if (!email || !password) { showAuthMessage('Enter email and password.', 'error'); return; }
      if (password !== confirm) { showAuthMessage('Passwords do not match.', 'error'); return; }
      ...
      try {
        ...
        if (error) {
          ...
          showAuthMessage('Signup successful! Check your email to confirm.', 'success');
          // show login form after signup
          setTimeout(() => { openAuthModal('login'); }, 1500);
        }
      } catch (err) {
        console.error(err);
        showAuthMessage('Unexpected error during signup', 'error');
      } finally {
        submitBtn.disabled = false;
      }
    });
  ...
  if (githubBtn) githubBtn.addEventListener('click', () => supabase.auth.signInWithOAuth({ provider: 'github' }));
  if (googleBtn) googleBtn.addEventListener('click', () => supabase.auth.signInWithOAuth({ provider: 'google' }));
  if (discordBtn) discordBtn.addEventListener('click', () => supabase.auth.signInWithOAuth({ provider: 'discord' }));
